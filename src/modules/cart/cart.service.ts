...
import { AddToCartInput, UpdateCartItemQtyInput } from "./cart.type";
...
export const CartService = {
    ...
    async addItemToCart(userId: string, payload: AddToCartInput) {
        ...
        const combo = await prisma.combo.findUnique({
            ...
            select: { id: true, price: true, stock: true, isActive: true, title: true },
        });
    ...
    async updateItemQty(userId: string, comboId: string, payload: UpdateCartItemQtyInput) {
        ...
        if (quantity > combo.stock) {
            throw new AppError(status.BAD_REQUEST, `Not enough stock. Available: ${combo.stock}`);
        }
